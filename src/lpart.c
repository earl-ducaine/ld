...
int show_part_table(void) {
  return 0;
}

int extract_part(char *partition) {
  ...
  for (i = 0; i < blocks; i++) {
    ...
    // TODO -- not retrying is an error.
    ssize_t write_result = write(fd2, block, bpblk);
    if (write_result != bpblk) {
      printf("lpart.c: unable to write block.");
    }
  }
  close(fd2);
  return 0;
...
int load_part(char *partition) {
  int i, index, blocks, fd2;
  off_t offset;
  ssize_t ret;
  index = -1;
  for (i = 0; i < part_table_size; i++) {
...
int active_part(char *partition __attribute__ ((unused)),
		int onoff __attribute__ ((unused))) {
#if 0
  struct part_table_s *pt;
  struct part_table_entry_s *pe;
